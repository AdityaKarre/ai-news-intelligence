...
.stApp {{
    ...
    background:
        radial-gradient(circle at top left, #262240 0%, #181825 45%);
...
.news-card.is-open {{
    ...
    border-color: rgba(180,167,255,0.24);

    box-shadow:
        0 0 0 1px rgba(183,166,255,0.10),
...
div[data-testid="stButton"] > button {{
    ...
    border:
        1px solid
        rgba(180,167,255,0.10);

    background:
        rgba(180,167,255,0.06);
...
.read-original-btn {{
    ...
    background:
        rgba(180,167,255,0.06) !important;

    border:
        1px solid
        rgba(180,167,255,0.10) !important;
...
.read-original-btn:hover {{
    ...
    background:
        rgba(180,167,255,0.12) !important;
...
for article in st.session_state.articles:
    ...
    is_open = (
        st.session_state.expanded_article_uid
        == article_id
    )
...
''',
        ...
        with btn_col1:
            ...
            context_clicked = st.button(
                "🔍 More Context",
                key=f"context_btn_{article_id}",
                use_container_width=True
            )
        ...
        if context_clicked:
            ...
            if context_key not in st.session_state:
                ...
                with st.spinner(
                    ...
                    context_clean = filter_relevant_content(
                        article["title"],
                        context_source
                    )
...
AI News Intelligence • Built with Streamlit & AI
